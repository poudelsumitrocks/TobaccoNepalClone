...
export default function Header() {
  ...
  const navMobile = (path) =>
    `text-lg cursor-pointer transition-all pb-1 border-b-2 ${
      pathname === path
        ? "border-[#eab308] text-[#eab308]"
        : "border-transparent text-white hover:text-[#eab308]"
    }`;
    const dropdownMobile=(path)=>`px-2 py-2 rounded-md cursor-pointer border-b-1${
      pathname===path?"border-[#eab308] text-[#eab308":"border-transparent text-white"
    }`
    ...
    <header className="w-full bg-[#211f1b] fixed top-0 left-0 z-50">
      <div className="h-22 flex items-center justify-between px-4 md:px-10">
        ...
        <nav className="hidden md:flex gap-6 text-white items-center">
          ...
          <div ref={dropdownRef} className="relative">
            ...
            {showDropdown && (
              <div className="absolute top-full mt-2 w-48 bg-[#211f1b] border border-[#eab308]/30 rounded-lg shadow-lg p-2">
                <Link
                  href="/gallery/photo"
          ...
          <button className="bg-[#eab308] px-6 py-2 rounded font-bold text-black hover:bg-[#d4a007]">
            Inquire Now
          </button>
        </nav>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#211f1b] text-white flex flex-col gap-4 px-6 py-6 border-t border-[#eab308]/20">
          <Link href="/" onClick={() => setMobileMenuOpen(false)} className={navMobile("/")}>Home</Link>
          <Link href="/product" onClick={() => setMobileMenuOpen(false)} className={navMobile("/product")}>Product</Link>
          <Link href="/about" onClick={() => setMobileMenuOpen(false)} className={navMobile("/about")}>About</Link>
          <Link href="/blog" onClick={() => setMobileMenuOpen(false)} className={navMobile("/blog")}>Blog</Link>
          <div  className="relative">
          <button
            onClick={() => setShowDropdown(!showDropdown)}
            className="flex items-center gap-1 text-lg transition-all"
          >
            Gallery {showDropdown ? <FaAngleUp /> : <FaAngleDown />}
          </button>
          </div>
          ...
          {showDropdown && (
            <div className="border-l-2 border-[#5e4b14] p-2">
              <div className="pl-2 flex flex-col gap-2">
                <Link href="/gallery/photo" onClick={() => setMobileMenuOpen(false)} className={dropdownMobile("/gallery/photo")}>
                  Photo Gallery
                </Link>
                <Link href="/gallery/video" onClick={() => setMobileMenuOpen(false)}className={dropdownMobile("/gallery/video")}>
                  Video Gallery
                </Link>
              </div>
            </div>
          ...
          <Link href="/contact" onClick={() => setMobileMenuOpen(false)} className={navMobile("/contact")}>
            Contact
          </Link>
